...
class DocumentProcessor:
    ...
    def __init__(self, region, role, bucket, document):
        self.region = region
        self.roleArn = role
        self.bucket = bucket
        self.document = document

        self.textract = boto3.client('textract', region=self.region)
        self.sqs = boto3.client('sqs', region=self.region)
        self.sns = boto3.client('sns', region=self.region)
...
def main():
    roleArn = '<INSERT ARN FROM STEP 3>'
    bucket = '<BUCKET NAME>'
    document = '<PATH/TO/PDF_FILE>'

    analyzer = DocumentProcessor(region, roleArn, bucket, document)
